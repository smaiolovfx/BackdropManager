    ...
    def __init__(self):
       ...
       # Format dropdown
        ...
        for idx, color in enumerate(self.colors):
            ...
            self.box_layout.addWidget(btn, r, self.c)
            self.c += 1
            if self.c == 4:
                self.c = 0
        ...
        button_reset = QtWidgets.QPushButton("Revert")
        button_reset.setToolTip("Clear changes and revert back to previous saved settings.")
        button_reset.clicked.connect(self.reset)
        box7.addWidget(button_reset)
        self.button_reset = button_reset

        # Close panel button
        button_close = QtWidgets.QPushButton("Ok")
...
class BackdropManagerUI(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.color_list = self.data['colors']
        try:
            baseCol = rgb2hex(self.color_list[0])
        except:
            baseCol = None
        self.colBox.setStyleSheet("background-color: %s;" % baseCol)
        model = self.colBox.model()
        for row, color in enumerate(self.color_list):
            self.colBox.addItem(rgb2hex(color))
    ...
    def changeColor(self):
        ...
        self.colBox.setStyleSheet("background-color: %s;" % color)
